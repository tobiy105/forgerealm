  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-4">
        <div
          className={`flex items-center justify-between transition-all duration-300 rounded-2xl border ${scrolled ? "py-2 px-4 bg-[#080c16] border-white/[0.06] shadow-xl shadow-black/30" : "py-3 border-transparent"}`}
        >
          {/* Left: Logo */}
          <a href="/" className="inline-flex items-center gap-3 group">
            <div
              className={`relative rounded-full p-1.5 transition-all duration-500 border ${scrolled ? "bg-black/60 backdrop-blur-xl border-[#FADE6A]/30 sm:border-white/10" : "border-transparent"}`}
            >
              <div className="relative">
                ...
                <img
                  ...
                  width={36}
                  height={36}
                  className="relative h-9 w-9 rounded-full transition-transform duration-300 group-hover:rotate-12 group-hover:drop-shadow-[0_0_12px_rgba(59,130,246,0.5)]"
                  loading="eager"
                  decoding="async"
                  fetchPriority="high"
                />
            ...
            <div className="hidden sm:block">
              <span
                className="text-[15px] font-semibold text-white block leading-tight whitespace-nowrap"
                style={{ fontFamily: "'Cinzel', serif" }}
              >
                ForgeRealm
              </span>
              <span
                className="text-[8px] uppercase tracking-[0.3em] text-blue-300/50 whitespace-nowrap"
          ...
          <div
            className={`sm:hidden absolute left-1/2 -translate-x-1/2 transition-all duration-500 ${scrolled ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2"}`}
          >
            <span
              className="text-[14px] font-semibold whitespace-nowrap text-[#FADE6A]"
              style={{ fontFamily: "'Cinzel', serif", color: "#FADE6A" }}
            >
              ForgeRealm
            </span>
          </div>
          ...
          <nav
            className={`hidden md:flex items-center gap-1 rounded-full px-2 py-1.5 transition-all duration-500 ${scrolled ? "bg-black/50 backdrop-blur-xl border border-white/[0.08]" : "bg-white/[0.04] backdrop-blur-sm border border-white/[0.06]"}`}
          >
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <HiOutlineMenu
                className={`h-5 w-5 transition-colors duration-500 ${scrolled ? "text-[#FADE6A]" : "text-white"}`}
              />
            </button>
          </div>
        </div>
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(180deg, #080c16 0%, #0c1222 50%, #080c16 100%)",
        }}
      >
        {/* Close button */}
        <div className="flex items-center justify-between p-6">
          ...
          <button
            onClick={() => setOpen(false)}
            className="rounded-full bg-white/[0.06] border border-white/10 p-2.5 text-white/60 hover:text-white transition"
